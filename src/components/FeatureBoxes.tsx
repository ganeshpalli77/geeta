    ...
    <section className="w-full px-4 sm:px-8 md:px-12 lg:px-20 relative z-10">
      <div className="flex flex-col sm:flex-row justify-center gap-4 max-w-7xl mx-auto divide-y-[2px] sm:divide-y-0 sm:divide-x-[2px] divide-white">
        {features.map((feature, index) => (
          ...
          >
            <h3 className="text-2xl md:text-2xl font-black uppercase" 
            style={{
              color: "#b24a31"
            }}
            >{feature.title}</h3>
            <button className="bg-[#8c2911] text-white px-5 md:px-6 py-2 rounded-[25px] transition-colors text-sm md:text-base font-bold" style={{ color: '#FFFFFF', backgroundColor: '#B54520' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#9C3B1B'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#B54520'}>
              {feature.buttonText}
            </button>
          </div>
        ))}
